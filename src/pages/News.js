...
const News = () => {
  ...
  const handleDelete = (id) => {
    setLoading(true); // Set loading state to true
    fetch(`http://localhost:8000/api/v1/news/delete/${id}`, {
      method: "DELETE",
      headers: {
        "content-type": "application/json",
      },
      ...
      .then((data) => {
        ...
        getNews(); // Fetch updated list after successful deletion
      })
      .catch((error) => {
      ...
      <div>
        ...
        >
          <div>
            ...
            <p>News are {news.length > 0 ? "available." : "not available."}</p>
          </div>
          <div>
        ...
        {loading ? (
          ...
          <Table dataSource={news} rowKey="_id">
            <Column
              title="Banner"
              dataIndex="banner"
              key="banner"
            ...
            <Column
              ...
              render={(_, record) => (
                <Space size="middle">
                  <Link to={`/news/edit-news/${record._id}`}>
                    <Button type="primary">
                      <EditOutlined />
                    </Button>
                  </Link>
